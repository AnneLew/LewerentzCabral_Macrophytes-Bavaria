...
```{r}
species_list <- PRESABS %>%
  summarise_if(is.numeric, sum, na.rm = TRUE) %>% gather("species","value",2:95)
```
...
Total number of submerged species 
```{r}
PRESABS_S %>% dplyr::select(`Alisma plantago-aquatica`:"Zannichellia palustris") %>% ncol()

```
Total number of emerged species 
```{r}
PRESABS_E[3:36] %>% ncol()
```
List with all species
```{r}
spec_sub<- PRESABS_S %>% dplyr::select(`Alisma plantago-aquatica`:"Zannichellia palustris") %>% gather("species","value",1:71) %>% dplyr::select(species) %>% unique()

spec_ef<- PRESABS_E[3:36]%>% gather("species","value",1:34) %>% dplyr::select(species) %>% unique()

spec <- rbind(spec_sub,spec_ef) %>% unique()
```
...
Number of Erscheinungsform
```{r}
spec_ana<-spec %>% left_join(species_info, by=c("species"="Taxon"))

spec_ana %>% group_by(Erscheinungsform) %>%
  summarise(nSpec=n())
```

Species richness mean
```{r}
Lakes_Type_centers %>% summarise(
  GAMMAm=mean(GAMMA),
  GAMMAsd=sd(GAMMA),
  GAMMA_SFm=mean(GAMMA_SF),
  GAMMA_SFsd=sd(GAMMA_SF),
  GAMMA_Em=mean(GAMMA_E),
  GAMMA_Esd=sd(GAMMA_E))
```

Species richness in natural vs artificial lakes 
```{r}
#Makroph_Lake
Lakes_Type_centers %>% group_by(Nat.artifi) %>% summarise(
  GAMMAm=mean(GAMMA),
  GAMMAsd=sd(GAMMA),
  GAMMA_SFm=mean(GAMMA_SF),
  GAMMA_SFsd=sd(GAMMA_SF),
  ...
  GAMMA_Esd=sd(GAMMA_E))
```
Number of species in Natural and Artificial lakes
```{r}
NatArti<-Makroph_Lake %>% left_join(result_Chem, by="Gewässer") %>% 
  dplyr::select(-GAMMA.x,-GAMMA.y) %>%
  group_by(Nat.artifi) %>% summarise_if(is.numeric, sum) #%>%

  
  specnumber(NatArti[3:96])
```

Emergent
```{r}
NatArtiE<-Makroph_LakeE %>% left_join(result_Chem, by="Gewässer") %>% 
  #dplyr::select(-GAMMA.x,-GAMMA.y) %>%
  group_by(Nat.artifi) %>% summarise_if(is.numeric, sum) #%>%

  
specnumber(NatArtiE[3:36])
```

Submerged
```{r}
NatArtiS<-Makroph_LakeSF %>% left_join(result_Chem, by="Gewässer") %>% 
  #dplyr::select(-GAMMA.x,-GAMMA.y) %>%
  group_by(Nat.artifi) %>% summarise_if(is.numeric, sum) #%>%

  
specnumber(NatArtiS[3:73])
```
...
```{r}
# species_info %>% filter(!is.na(S)) %>% nrow()
# species_info %>% filter(!is.na(Em)) %>% nrow()
# species_info %>% filter(!is.na(`F-SB`)) %>% nrow()
```
...
```{r}
Tab2<-Lakes_Type_centers %>% dplyr::select(ID, SEE_NAME, YEAR, GAMMA_SF, GAMMA_E, GAMMA) %>%
...
# library(Rarity)
# 
# W=rWeights(t(PRESABS_S[3:73]),wMethods = "W", rCutoff = "Gaston")
# Irr(t(PRESABS_S[3:73]),W, abundance=F)
# 
# W=rWeights(spid.occ,wMethods = "W", rCutoff = "Gaston")
# Irr(spid.occ,W, abundance=T)
# 
# 
# 
# library(remotes) # rare7
# 
# rareData(PRESABS_S[3:73])
# 
# ####
# rankabunplot(rankabundance(PRESABS_S[3:73]), main="submerse Arten", specnames=c(1,2,3),scale='abundance')
# rankabunplot(rankabundance(PRESABS_E[3:36]), main="emerse Arten", specnames=c(1,2,3),scale='abundance')
...
## 
```{r}
# resultSFm<-merge(resultSF, morph, by.x="Gewässer", by.y="Name_Makro_short")%>%
#   filter(!Gewässer %in% c("Eibsee", "Grosser Brombachsee", "Liebensteinspeicher","Murnersee","Steinberger See"))
# 
# resultSFm %>% #%>% spread(YEAR,GAMMA_Chara)
#   ggplot(aes(x=YEAR,y=meanQuant_CHARA,col=Type_simpl))+
#   geom_path(aes(group=Gewässer)) +
#   theme(legend.position = "bottom")+
#   facet_wrap(~Type_simpl, scales = "free_y")#+
#   #stat_summary(fun.data=meanQuant_CHARA) + 
...
#   mutate(change = (meanQuant_CHARA/lag(meanQuant_CHARA)-1)*100) %>%
#   filter(change!="NA") %>%
#   mutate(changeType=ifelse(change=="NaN", "nochange",
#                        ifelse(change>0,"increase",
#                               ifelse(change<0,"decrease",NA)))) %>%
#   ungroup() 
# 
# merge(R, Chem_surf, by.x=c("Name_chem","YEAR"), by.y=c("Gewässer","YEAR"))%>% group_by(changeType) %>%
